...
void CBaseCombatCharacter::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget /* = NULL */, const Vector *pVelocity /* = NULL */ )
{
	...
	if ( IsPlayer() )
	{
		Vector vThrowPos = Weapon_ShootPosition() - Vector(0,0,10);
	...
	else
	{
		...
		{
			// Nowhere in particular; just drop it.
			float throwForce = ( IsPlayer() ) ? 400.0f : random->RandomInt( 64, 108 );
...
//
// A weapon is said to be on the ground if it is no more than 10 inches above
...
bool CBaseCombatCharacter::Weapon_IsOnGround( CBaseCombatWeapon *pWeapon )
{
	...
	if( fabs(pWeapon->WorldSpaceCenter().z - GetAbsOrigin().z) >= 10.0f )
	{
		return false;
	}
...
void RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrc, float flRadius, int iClassIgnore, CBaseEntity *pEntityIgnore )
{
	...
	if( info.GetDamageType() & DMG_BLAST )
	{
		...
		// be less than 108 units.
		float soundRadius = MAX( 108.0f, flRadius * 1.5 );
