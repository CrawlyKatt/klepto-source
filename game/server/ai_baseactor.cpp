	...
	case CChoreoEvent::FACE: 
		{
			...
			NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 8 ), GetLocalAngles().y + m_goalBodyYaw, 64, 16, 255, 108, 108, 0, true, 0.1 );
			NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 16 ), GetLocalAngles().y + m_goalSpineYaw, 64, 16, 108, 255, 108, 0, true, 0.1 );
			NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 24 ), info->m_flTargetYaw, 64, 16, 108, 108, 255, 0, true, 0.1 );
			*/
			...
			{
				NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 1 ), goalYaw, 8 + 32 * intensity, 8, 255, 255, 255, 0, true, 0.10 );
			}
...
void CAI_BaseActor::UpdateHeadControl( const Vector &vHeadTarget, float flHeadInfluence )
{
	...
	if (iChest > 0)
	{
		...
		if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
		{
			...
			NDebugOverlay::Line( tmp1, tmp2, 0,0,255, false, 0.10 );
...
bool CAI_BaseActor::PickRandomLookTarget( AILookTargetArgs_t *pArgs )
{
	...
	if ( bIsNavigating && random->RandomInt(1, 10) <= 3 )
	{
		Vector navLookPoint;
		Vector delta;
		if ( GetNavigator()->GetPointAlongPath( &navLookPoint, 10 * 10 ) && (delta = navLookPoint - GetAbsOrigin()).Length() > 8.0 * 10.0 )
		{
	...
	int iConsidered = 0;
	for ( CEntitySphereQuery sphere( GetAbsOrigin(), 30 * 10, 0 ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		if (pEntity == this)
		{
			continue;
...
#endif // HL2_EPISODIC
	{
		pArgs->vTarget = EyePosition() + forward * 108 + right * random->RandomFloat(-32,32) + up * random->RandomFloat(-16,16);
	}
...
void CAI_BaseActor::MaintainLookTargets( float flInterval )
{
	...
	if (!bFoundTarget && !ValidEyeTarget( GetViewtarget() ))
	{
		...
		// DevMsg("random view\n");
		SetViewtarget( EyePosition() + HeadDirection3D() * 108 + right * random->RandomFloat(-32,32) + up * random->RandomFloat(-16,16) );
	}
