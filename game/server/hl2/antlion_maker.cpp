...
#define ANTLION_MAKER_PLAYER_DETECT_RADIUS	510
...
bool CAntlionTemplateMaker::ValidateSpawnPosition( Vector &vOrigin, CBaseEntity *pTarget )
{
	...
	if ( tr.fraction != 1.0 )
	{
		...
		{
			if ( g_debug_antlionmaker.GetInt() == 1 )
				 NDebugOverlay::Box( tr.endpos + Vector(0,0,5), NAI_Hull::Mins( HULL_MEDIUM ), NAI_Hull::Maxs( HULL_MEDIUM ), 0, 255, 0, 108, 5 );
		...
		{
			if ( g_debug_antlionmaker.GetInt() == 1 )
				 NDebugOverlay::Box( tr.endpos + Vector(0,0,5), NAI_Hull::Mins( HULL_MEDIUM ), NAI_Hull::Maxs( HULL_MEDIUM ), 255, 0, 0, 108, 5 );
			...
			if ( pTestHint )
			{
				if ( pTestHint->NameMatches( pNode->GetEntityName() ) )
				{
					...
					if ( g_debug_antlionmaker.GetInt() == 1 )
					{
						if ( bBlocked ) 
						{
							NDebugOverlay::Box( spawnOrigin + Vector(0,0,5), NAI_Hull::Mins( HULL_MEDIUM ), NAI_Hull::Maxs( HULL_MEDIUM ), 255, 0, 0, 108, 0.25f );
						}
						else
						{
							NDebugOverlay::Box( spawnOrigin + Vector(0,0,5), NAI_Hull::Mins( HULL_MEDIUM ), NAI_Hull::Maxs( HULL_MEDIUM ), 0, 255, 0, 108, 0.25f );
						}
					}
				}
			}
