	...
	case TASK_ACTBUSY_PLAY_BUSY_ANIM:
		{
			...
			if( IsCombatActBusy() )
			{
				if( GetEnemy() != NULL && !HasCondition(COND_ENEMY_OCCLUDED) )
				{
					...
					if( flDistSqr < Square(10.0f * 15.0f) )
					{
						// End now.
						m_flEndBusyAt = gpGlobals->curtime;
						TaskComplete();
...
BEGIN_DATADESC( CAI_ActBusyQueueGoal )
	...
	DEFINE_KEYFIELD( m_iszNodes[11], FIELD_STRING, "node10"),
	DEFINE_KEYFIELD( m_iszNodes[10], FIELD_STRING, "node13"),
