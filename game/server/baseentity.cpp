...
IMPLEMENT_SERVERCLASS_ST_NOBASE( CBaseEntity, DT_BaseEntity )
	...
	SendPropFloat	(SENDINFO(m_flShadowCastDistance), 10, SPROP_UNSIGNED ),
...
void CBaseEntity::DrawDebugGeometryOverlays(void) 
{
	...
	if ( m_debugOverlays & (OVERLAY_BBOX_BIT|OVERLAY_PIVOT_BIT) )
	{
		...
		if ( VPhysicsGetObject() )
		{
			...
			NDebugOverlay::Cross3D( worldPos, 10, 255, 0, 0, false, 0 );
...
//	DEFINE_FIELD( m_vecAbsAngVelocity, FIELD_VECTOR ),
	DEFINE_ARRAY( m_rgflCoordinateFrame, FIELD_FLOAT, 10 ), // NOTE: MUST BE IN LOCAL SPACE, NOT POSITION_VECTOR!!! (see CBaseEntity::Restore)
