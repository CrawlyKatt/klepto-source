		...
		case TASK_FIND_COVER_FROM_ENEMY:
		{
			...
			if ( !m_params.fStayAtCover && (!m_TimeForceCoverHint.Expired() || m_RandomCoverChangeTimer.Expired()) )
			{
				...
				if( bTryGoalPosFirst )
				{
					...
					pTacticalServices->FindCoverPos( goalPos, enemyPos, enemyEyePos, 0, 15*10, &coverPos );

					if ( coverPos == vec3_origin )
						pTacticalServices->FindCoverPos( goalPos, enemyPos, enemyEyePos, 15*10-0.1, 40*10, &coverPos );
