...
#define RAPPEL_DECEL_DIST	(20.0f * 10.0f)	// Start slowing down when you're this close to the ground.
...
void CAI_RappelBehavior::BeginRappel()
{
	...
	if( tr.m_pEnt != NULL && tr.m_pEnt->IsNPC() )
	{
		...
		CSoundEnt::InsertSound( SOUND_DANGER, tr.m_pEnt->EarPosition() - forward * 10.0f, 32.0f, 0.2f, GetOuter() );
	}
}
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER( CAI_RappelBehavior )
	...
	(
		...
		"	Tasks"
		"		TASK_HIT_GROUND			0"
		"		TASK_MOVE_AWAY_PATH		108"	// Clear this spot for other rappellers
		"		TASK_RUN_PATH			0"
