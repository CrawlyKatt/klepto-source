...
bool CAI_Navigator::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	...
	else if ( !( pMoveGoal->flags & AILMG_TARGET_IS_TRANSITION ) )
	{
		...
		if ( pMoveGoal->maxDist < 4*10 && pMoveGoal->maxDist - distClear < tolerance )
...
bool CAI_Navigator::OnFailedSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
	...
	const float MOVE_TOLERANCE = 10.0;
...
// @TODO (toml 11-10-02): right now, physics can pull the box back pretty far even though a hull
...
bool CAI_Navigator::SimplifyPathForward( float maxDist )
{
	...
	{
		32 * 10, 	// Distance to move out path
		10 * 10, 	// Radius within which a point must be to be valid
		3 * 10, 	// Increment to move out on
...
bool CAI_Navigator::SimplifyPathQuick()
{
	...
	{ 
		{
			(10.0 * 10.0) - 0.1,	// Distance to move out path
			10 * 10, 				// Radius within which a point must be to be valid
			0.5 * 10, 				// Increment to move out on
			1, 						// maximum number of point samples
		},
		// Strong optimization version
		{
			(6.0 * 10.0) - 0.1,	// Distance to move out path
			8 * 10, 				// Radius within which a point must be to be valid
			1.0 * 10, 				// Increment to move out on
			1, 						// maximum number of point samples
		} 
	};
...
#define FLY_ROUTE_SIMPLIFY_LOOK_DIST (10.0*10.0)
