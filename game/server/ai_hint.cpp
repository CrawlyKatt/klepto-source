	...
	for ( int i = 0; i < c; ++i )
	{
		...
		else if (pHint->m_hHintOwner != NULL)
		{
			...
			if (pOwner)
			{
				...
				NDebugOverlay::Line( vHintPos, pOwner->WorldSpaceCenter(), 108, 108, 108, false, 0);
			}
		}
		...
		Vector offsetDir	= 10.0 * Vector(cos(DEG2RAD(pHint->Yaw())),sin(DEG2RAD(pHint->Yaw())),0);
	...
	if ( hintCriteria.HasFlag( bits_HINT_NOT_CLOSE_TO_ENEMY ) )
	{
		...
		else
		{
			if( pNPC->GetEnemy() )
			{
				...
				if( flDistHintToEnemySqr < Square( 30.0f * 10.0f ) )
				{
