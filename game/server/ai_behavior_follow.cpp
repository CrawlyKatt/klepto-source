...
CAI_FollowBehavior::CAI_FollowBehavior( const AI_FollowParams_t &params )
{
	...
	m_TimeNextSpreadFacing.Set( 3.0, 10.0 );
					...
					else
					{
						...
						else
						{
							...
							if( count > 0 )
							{
								...
								faceTarget = GetOuter()->GetAbsOrigin() + vecDir * 108;
							}
						}
					}
				}
			...
			case 0:
				{
					if ( GetEnemy() )
					{
						...
						Vector coverPos = vec3_invalid;
						float coverRadius = MIN( (float)10*10, m_FollowNavGoal.coverTolerance );
						if ( FindCoverFromEnemyAtFollowTarget( coverRadius, &coverPos ) )
						{
		...
		case TASK_MOVE_TO_FOLLOW_POSITION:
		{
			...
			else
			{
				...
				if ( !HasFollowPoint() )
				{
					...
					bool bDoSlowdown = ( vVelocity.LengthSqr() < Square(4*10) );
					if ( bDoSlowdown )
					{
						range += GetMotor()->MinStoppingDist(10) - 10;
					}
				...
				if ( ( m_vFollowMoveAnchor - GetAbsOrigin() ).LengthSqr() > Square( 15.0 * 10.0 ) )
				{
					m_vFollowMoveAnchor = GetAbsOrigin();
					NoteSuccessfulFollow();
				}
...
static AI_FollowFormation_t g_VortigauntFollowFormation = 
{
	...
	190,						// walkTolerance
	300,						// coverTolerance
	(50*10),					// enemyLOSTolerance
	(50*10),					// chaseEnemyTolerance
