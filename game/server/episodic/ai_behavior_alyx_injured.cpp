...
#define	COVER_DISTANCE	108.0f			// Distance behind target to find cover
...
void CAI_BehaviorAlyxInjured::GatherConditions( void )
{
	...
	if ( AI_IsSinglePlayer() && bInDanger )
	{
		...
		if ( bWarnPlayer )
		{
			...
			//if ( HasCondition( COND_SEE_PLAYER ) == false || flPlayerDistSqr > Square( 108 ) )
			{
